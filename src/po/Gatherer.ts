...
export function poGather(config) {
  ...
  Object.keys(config.languages).forEach(lang => {
    ...
    let functionWrappers: RegexNamedGroups[] = config.functions.map((f) => new RegexNamedGroups(f, "gm"));
    ...
    fs.writeFileSync(filePath, oldfile.merge(newfile).generate(language_code, language_name, meta, config.keepOldMessages));
  });
};